...
class DotProdAttention(nn.Module):
    def __init__(self, encoder_dim):
        ...
        self.encoder_dimension = encoder_dim
        self.softmax = torch.nn.Softmax(1)

    def forward(self, encoder_output, hidden_state):
        # Verify sizes
        hidden_state = hidden_state.unsqueeze(2)
        allignment_scores = torch.matmul(encoder_output, hidden_state)
        soft_alignment_score = self.softmax(allignment_scores)
        context = encoder_output * soft_alignment_score / self.encoder_dimension
        return context, soft_alignment_score.squeeze()
